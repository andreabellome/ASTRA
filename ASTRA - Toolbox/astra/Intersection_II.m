...
% this function computes the anomaly difference between the SC and the planet at the second intersection
...
if isempty(find(pltsT == plT,1))  % you do not arrive at plT
    ...
    if kep(2) >= 1 % you are on hyperbolic orbit
        ...
        tof_2  = TOFs(2);  % extract the time of flight towards second intersections
        
        [rrIN_2, vvIN_2] = FGKepler_dt(kep, tof_2, mu); % compute SC state at the intersection
        
        tPL = tIN + tof_2/86400; % compute the epoch at the intersection
