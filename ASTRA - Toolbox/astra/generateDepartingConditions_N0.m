function [MMAT_2, DV0, meshTDEP, meshTOF] = ...
    generateDepartingConditions_N0(s, DEP_OPTIONS, ARR_OPTIONS, vinfMin, vinfMax)

% this function generates the first leg for ASTRA toolbox
% (only direct transfers - no multiple revolutions on the first leg)
...
for indi = 1:length(tt0)
    ...
    for indj = 1:length(TT)
        ...
        % one revolution
        [~,~,~,~,v1Short, v2Short,~,~] = lambertMR(r1, r2, TOF*86400, mu, 0, 0);
        dvShort = norm(v1Short - v1);
        
        [~,~,~,~,v1Long, v2Long,~,~] = lambertMR(r1, r2, TOF*86400, mu, 1, 0);
    ...
    % update the waitbar
    w = waitbar(indi/length(tt0), f, sprintf('Computing first leg - N0...'));
...
MMAT_2 = MAT_2;
% MMAT_2 = sortrows(MMAT_2, 9, 'ascend');
