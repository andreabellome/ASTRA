function [DM, rrIN_2, vvIN_2, tPL] = Intersection_I(H, gam, rrIN, vvIN, plIN, tIN, plT, planetsList)

% this function computes the anomaly difference between the SC and the planet at the first intersection
...
if isempty(find(pltsT == plT,1)) % you do not arrive at plT
    ...
    if kep(2) >= 1 % you are on hyperbolic orbit
        ...
        tof_1  = TOFs(1); % extract the time of flight towards first intersections
        
        [rrIN_2, vvIN_2] = FGKepler_dt(kep, tof_1, mu); % compute SC state at the intersection
        
        tPL = tIN + tof_1/86400; % compute the epoch at the intersection
